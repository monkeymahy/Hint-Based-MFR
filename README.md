...
- 凸台必须有可解释的顶面；只有侧壁或过渡连接段、但没有顶面证据的结构不会被识别为 `boss`。
...
- 如果顶面被孔穿过，顶面可以带内环，仍然可以作为 boss 的顶面。
- 如果顶面和侧壁之间有简单锥形倒角，程序允许通过这个倒角向后一跳找到顶面；倒角本身仍按 `chamfer` 处理。

最后一条用于修复圆柱凸台顶面漏识别的问题。它不依赖圆柱轴线本身的正负方向，因为 OCC 给出的圆柱轴方向可能是任意的；它改用 carrier 法向和面中心位置判断“顶面是否真的在凸台外侧”。

局部圆柱凸台 fallback 仍然保持保守：有明确 carrier 时也必须找到顶面；没有明确 carrier 时，只加入简单圆形端面，避免把独立圆柱的两个端面或普通连接段误收进 boss。
...
对于没有明显圆柱侧壁的平面小凸台，程序还有一个保守 fallback：候选平面组件必须和内环结构有关，不能是凹陷平面环，几何上要同时包含近似平行和近似正交的法向关系，并且能通过内环 carrier 找到向外的顶面。这样可以避免把普通外边界识别成 `boss`。
